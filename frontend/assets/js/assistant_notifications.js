...
function applyNotificationsData(data) {
    ...
    if (data.user && document.getElementById('assistantName')) {
        document.getElementById('assistantName').textContent = (isAr && window.i18n) ? window.i18n.translateName(data.user.name) : data.user.name;
    }
                ...
                '<div style="display:flex; justify-content:space-between; align-items:flex-start; gap:15px; flex-wrap:wrap;">' +
                    '<div style="flex:1;">' +
                        '<div style="display:flex; align-items:center; gap:10px; margin-bottom:6px;">' +
                            ...
                            (!n.is_read ? '<span class="status-badge" style="background:var(--role-assistant-bg); color:var(--role-assistant-text); font-size:11px;">' + (isAr ? 'جديد' : 'New') + '</span>' : '') +
                        '</div>' +
