...
if (isPost()) {
    ...
    if ($action === 'mark_read') {
        ...
        if ($notificationId > 0) {
            mysqli_query($conn, "UPDATE notifications SET is_read = 1 WHERE id = $notificationId AND user_id = $studentId");
        }
    ...
    if ($action === 'mark_all_read') {
        mysqli_query($conn, "UPDATE notifications SET is_read = 1 WHERE user_id = $studentId");
        echo json_encode(['success' => true]);
        exit;
    }
